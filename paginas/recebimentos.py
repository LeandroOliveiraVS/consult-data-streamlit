...
def recebimentos_page():
        ...
        try:
            ...
            with _engine.connect() as connection:
                ...
                if coluna_data in df.columns:
                    ...
                    st.warning(
                        f"Atenção: A coluna de data '{coluna_data}' não foi encontrada na tabela."
                    )
                # ============================== CONVERSÕES =====================================
                ...
                if coluna_hora in df.columns:
                    ...
                    st.warning(
                        f"Atenção: A coluna de hora '{coluna_hora}' não foi encontrada na tabela."
                    )
                # ===============================================================================
            return df
    ...
    st.markdown(
        "Use os filtros na barra lateral para explorar os dados de recebimentos."
    )
    ...
    if (
        ...
        and not df[coluna_data].isnull().all()
    ):
        ...
        df_filtrado = df_filtrado[
            df_filtrado[coluna_data].dt.date.between(data_inicio, data_fim)
        ]

    else:
        st.sidebar.warning(
            f"Não foi possível criar o filtro de data. Verifique a coluna '{coluna_data}'."
        )
